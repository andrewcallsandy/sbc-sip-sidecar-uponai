...
function pickRelevantCarrierProperties(c) {
  return {
    ...
    register_expires: c.register_expires || null
  };
}
  ...
  try {
    ...
    if (active) {
      if (token === myToken) {
        ...
        if (srf.locals.blf?.stopAll) {
          srf.locals.blf.stopAll(logger, srf).catch((err) => {
            logger.info({err}, 'BLF stopAll on stand-down');
          });
        }
        ...
        if (srf.locals.blf?.stopAll) {
          srf.locals.blf.stopAll(logger, srf).catch((err) => {
            logger.info({err}, 'BLF stopAll before re-claim');
          });
        }
        regbots.forEach((rb) => rb.stop(srf));
        regbots.length = 0;
    ...
    if (grabForTheWheel) {
      ...
      if (result === 'OK') {
        ...
        if (srf.locals.blf?.start) srf.locals.blf.start(logger, srf);
      }
      else {
        srf.locals.regbot.active = false;
      ...
      for (const gw of getUniqueGateways(gws, logger)) {
        ...
        try {
          const opts = {
            ...
            register_expires: gw.carrier.register_expires
          };
