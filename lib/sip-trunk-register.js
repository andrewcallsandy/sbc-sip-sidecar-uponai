...
function pickRelevantGatewayProperties(gw) {
  return {
    sip_gateway_sid: gw.sip_gateway_sid,
    ipv4: gw.ipv4,
    port: gw.port,
    inbound: gw.inbound,
    outbound: gw.outbound,
...
const updateCarrierRegbots = async(logger, srf) => {
  ...
  try {
    ...
    const cs = (await lookupAllVoipCarriers())
      ...
      .sort((a, b) => a.voip_carrier_sid.localeCompare(b.voip_carrier_sid));
    ...
    gws.sort((a, b) => a.sip_gateway_sid.localeCompare(b.sip_gateway_sid));
    const gwsCompare = gws.map((gw) => pickRelevantGatewayProperties(gw));
    const gatewaysCompare = gateways.map((gw) => pickRelevantGatewayProperties(gw));
    if (JSON.stringify(gwsCompare) !== JSON.stringify(gatewaysCompare)) hasChanged = true;
      ...
      for (const gw of getUniqueGateways(gateways, logger)) {
        ...
        if (!gw.carrier.register_public_ip_in_contact && gw.carrier.account_sid) {
          ...
          if (account && account.sip_realm) {
            account_sip_realm = account.sip_realm;
          }
        }
        try {
