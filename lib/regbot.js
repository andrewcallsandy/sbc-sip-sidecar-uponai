...
const {
  ...
  JAMBONES_REGISTRATION_WEBHOOK_URL
} = require('./config');
...
const REGISTER_FAILURE_THRESHOLD = (parseInt(JAMBONES_REGBOT_REGISTER_FAILURE_THRESHOLD) || 6);
...
class Regbot {
  constructor(logger, opts) {
    ...
    this.defaultExpires = opts.register_expires || DEFAULT_EXPIRES;
    ...
    try {
      ...
      const req = await srf.request(`${scheme}:${this.sip_realm}`, {
        ...
        headers: {
          ...
          'From': this.from,
          'To': this.from,
          'Contact': `<${scheme}:${contactAddress};transport=${transport}>;expires=${this.defaultExpires}`,
      ...
      req.on('response', async(res) => {
        ...
        if (res.status !== 200) {
          ...
          if (REGISTER_RESPONSE_REMOVE.includes(res.status)) {
            ...
            this.logger.info(
              `${this.aor}: consecutive remove-failures: ` +
              `${this.consecutiveRemoveFailures}/${REGISTER_FAILURE_THRESHOLD}`
            );
            if (this.consecutiveRemoveFailures >= REGISTER_FAILURE_THRESHOLD) {
              ...
              if (carrier) {
                let event;
                let message;
                if (carrier.trunk_type !== 'reg') {
                  await updateCarrierBySid(this.voip_carrier_sid, {requires_register: false});
                  event = 'registration-disabled';
                  message =
                    `Disabling Outbound Registration for carrier ${carrier.name} ` +
                    `(sid:${carrier.voip_carrier_sid})`;
                } else {
                  await updateCarrierBySid(this.voip_carrier_sid, {is_active: false});
                  event = 'registration-deactivated';
                  message =
                    `Deactivating carrier ${carrier.name} ` +
                    `(sid:${carrier.voip_carrier_sid}) due to registration errors`;
                }
                ...
                if (JAMBONES_REGISTRATION_WEBHOOK_URL) {
                  fetch(JAMBONES_REGISTRATION_WEBHOOK_URL, {
                    method: 'POST',
                    ...
                    body: JSON.stringify({
                      ...
                      account_sid: carrier.account_sid,
                      service_provider_sid: carrier.service_provider_sid,
                      sip_status: res.status,
                      aor: this.aor,
                      gateway: `${this.ipv4}:${this.port}`,
                      timestamp: new Date().toISOString()
                    })
                  }).catch((err) => {
                    this.logger.info({err}, 'Error sending registration webhook');
                  });
                }
              }
            }
      ...
      if (REGISTER_RESPONSE_REMOVE.length > 0) {
        this.consecutiveRemoveFailures++;
        this.logger.info(
          `${this.aor}: consecutive remove-failures (DNS/network): ` +
          `${this.consecutiveRemoveFailures}/${REGISTER_FAILURE_THRESHOLD}`
        );
        if (this.consecutiveRemoveFailures >= REGISTER_FAILURE_THRESHOLD) {
          const { updateCarrierBySid, lookupCarrierBySid } = srf.locals.dbHelpers;
          ...
          if (carrier) {
            ...
            if (carrier.trunk_type !== 'reg') {
              ...
              message =
                `Disabling Outbound Registration for carrier ${carrier.name} ` +
                `(sid:${carrier.voip_carrier_sid}) due to repeated DNS/network failures`;
            } else {
              await updateCarrierBySid(this.voip_carrier_sid, {is_active: false});
              event = 'registration-deactivated';
              message =
            ...
            if (JAMBONES_REGISTRATION_WEBHOOK_URL) {
              fetch(JAMBONES_REGISTRATION_WEBHOOK_URL, {
                ...
                this.logger.info({err: fetchErr}, 'Error sending registration webhook');
              });
            }
          }
          updateVoipCarriersRegisterStatus(this.voip_carrier_sid, JSON.stringify({
